(function() {
	...
	document.addEventListener('deviceready', function(event) {
		...
		function onSuccess(imageData) {
			...
			addPhotoData(image.src, imageReceiptId);
    ...
    function addPic(table,data){
      ...
      console.log(pic_uri);
      Splitrdb.transaction(function(transaction){
        var executeSql = 'UPDATE '+table1+' set pic_uri = ' + "'" + pic_uri "'" + " where receipt_id = "+ "'" + receipt_id + "'", [], function(tx,results){
    ...
    function viewDataAll(table) {
      ...
      Splitrdb.transaction(function(transaction) {
        transaction.executeSql('SELECT * FROM '+table1, [], function (tx, results) {
    ...
    function viewDataReceipt(table){
      ...
      Splitrdb.transaction(function(transaction){
        transaction.executeSql('SELECT name, sum(price) as owed where receipt_id =' + receipt_id + 'group by name', [], function(tx,results) {
          var resLen = results.rows.length;
          for (i = 0; i < resLen; i++){ 
